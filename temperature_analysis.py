...
# 1) All city historical trends
ax = axes[0, 0]
for i, city in enumerate(city_labels):
    ax.plot(years, A[:, i], lw=1.2, label=city)
ax.set_title("Historical Temperature Trends")
ax.set_xlabel("Year")
ax.set_ylabel("Temperature (degC)")
ax.legend(fontsize=8, ncol=2)

# 2) Focus city actual vs fitted + future
ax = axes[0, 1]
ax.plot(years, A[:, focus_idx], label=f"{focus_city} Actual", lw=1.5)
ax.plot(years, A_pred[:, focus_idx], label=f"{focus_city} Fitted", lw=1.5)
ax.plot(future_years, A_future[:, focus_idx], "--", label=f"{focus_city} Forecast", lw=1.8)
ax.axvline(years[-1], color="black", linestyle=":", lw=1)
ax.set_title(f"Actual vs Predicted ({focus_city})")
ax.set_xlabel("Year")
ax.set_ylabel("Temperature (degC)")
ax.legend(fontsize=8)

# 3) Projection residual magnitude per city
ax = axes[1, 0]
city_residual = np.linalg.norm(projection_residual, axis=0)
ax.bar(city_labels, city_residual, color="teal", alpha=0.8)
ax.set_title("Projection Residual Norm by City")
ax.set_ylabel("Residual norm")
ax.tick_params(axis="x", rotation=20)

# 4) Scree plot
ax = axes[1, 1]
ax.bar(np.arange(1, len(eigvals) + 1), explained, color="coral", alpha=0.85)
ax.set_title("PCA Scree Plot")
ax.set_xlabel("Principal Component")
ax.set_ylabel("Variance Explained (%)")

fig.suptitle(
    f"Temperature Trend Analysis (Reference-Style) | Focus City: {focus_city}",
    fontsize=14,
    fontweight="bold",
)
...
section("DEMO ONE-LINERS")
print('Matrix: "Converted real data into matrix form"')
print('RREF: "Found independent columns using pivots"')
print('Basis: "Removed redundant information"')
print('Orthogonalization: "Created perpendicular basis for stable projection"')
print('Projection: "Reduced noise and preserved trend"')
print('Least Squares: "Estimated best-fit trend and forecast"')
print('PCA: "Detected dominant global warming pattern"')
print('Final Output: "Exported prediction and improved datasets"')
print("Conclusion: Temperatures show increasing trend across cities.")
